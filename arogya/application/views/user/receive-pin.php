...
<html lang="en">
<head>
  	<title><?=$page['page']='Received PINs';?> | <?=$this->siteInfo['name'];?></title>
	      	...
	      	<div class="container-fluid">
                ...
                <div class="panel panel-primary">
                  <div class="panel-heading">
                    <h3 class="panel-title">Receive PIN Status</h3>
                  </div>
                  <div class="panel-body">
                    <table id="table1" class="table table-bordered table-striped">
                      ...
                      <tbody>
                      <?php
                        $res1=$this->dbm->globalSelect('pin',['receiver_id'=>$this->logged['user_id']]);
                        $res2=$this->dbm->globalSelect('pin',['user_id'=>$this->logged['user_id'],'generate_by'=>'admin','is_transfer!='=>1]);
                        ...
                        <tr>
                          ...
                          <td><?=$value['pin'];?></td>
                          <td>
                            <?php if($value['generate_by']=='admin' && $value['is_transfer']!=1){ ?>
                              <?=$this->dbm->dateFormat($value['gen_date']);?> <?=$value['gen_time'];?>
                            <?php }else { ?>
                              <?=$this->dbm->dateFormat($value['transfer_date']);?> <?=$value['transfer_time'];?>
                            <?php } ?>
                          </td>
                          <td><?=($value['is_transfer']==1)?$value['user_id']:'Admin';?></td>
                          <td>
                          ...
                          <td><?php
                          if($value['generate_by']=='admin' && $value['is_transfer']==1 && $value['receiver_id']!=$this->logged['user_id'])
                          {
                            echo "Transfered to- ".$value['receiver_id'];
                          }else
                          {
