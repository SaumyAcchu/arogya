			...
			<div class="col-lg-12" id="treeDiv" style="width:100%;height:500px;overflow:scroll;">
				<div class="tree" style="position: static;">
					<ul>
						<li>
							<?php
							$userCount=$this->db_model->rowCount('flat_registration',['introducer'=>$this->logged['user_id']]); ?>
									<a href="#" class="" onmouseover="myfunction('<?=$this->logged['user_id']; ?>','<?=$this->logged['name']; ?>','<?=$userCount; ?>')" onmouseout="myfunction1()">
											<?=$key=$this->logged['user_id'];?> | 
										  Users <span class="badge badge-light"><?=$userCount;?></span>
									</a>
						...
						{ $i=1; krsort($data);
							...
							{ ?>
								<li>
									<?php
										$data = new db_model();
										//$userCount=db_model::rowCount('flat_registration',['introducer'=>$value['user_id']]);
										$userCount=$data->rowCount('flat_registration',['introducer'=>$value['user_id']]); ?>
									<a href="#" class="" onmouseover="myfunction('<?=$value['user_id']; ?>','<?=$value['name']; ?>','<?=$userCount; ?>')" onmouseout="myfunction1()">
											<?=$value['user_id'];?> | 
										  Users <span class="badge badge-light"><?=$userCount;?></span>
									</a><?php
