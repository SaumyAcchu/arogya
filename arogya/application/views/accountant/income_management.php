...
<html lang="en">
<head>
  	<title><?=$page['page']='income Management';?> | <?=$this->siteInfo['name'];?></title>
	    ...
	    <div class="right-container" id="right-container">
	      	<div class="container-fluid">
		        ...
		        <div class="row padding-top">
		             <center><button data-toggle="collapse in" class="btn btn-primary" data-target="#demo">New income Entry</button></center>	
					<div id="demo" class="collapse in ">
					<div class="col-lg-6 col-lg-offset-3" style="padding: 10px; box-shadow: 0px 1px 10px 1px #33A5E7; margin-top: 10px;">
						<?= form_open('accountant/incomeManagement/add',['class'=>'form-horizontal']); ?>
						<legend>Insert New income Voucher</legend>
					    <div class="form-group">
					      <label for="select" class="col-lg-3 control-label">Select Group</label>
					      <div class="col-lg-9">
					        <select name="income_category" class="form-control" required>
					    ...
					    <div class="form-group">
					      ...
					      <div class="col-lg-9">
					        <input type="number" value="" name="amount" class="form-control date" id="" placeholder="Enter Amount" required>
					      </div>
					    </div>
					    <div class="form-group">
					...
					<?php if(isset($data)):  ?>
						<div class="col-lg-10 col-lg-offset-1">
							...
							<table id="table1" class="table table-bordered table-striped table-hover table-condensed" style="margin-top: 30px;">
							  <thead>
							    <tr>
							      <th>Sl</th>
							      <th>Group</th>
							  ...
							  <tbody>
							  <?php if(count($data)) { ?>
							  <?php $i=1; foreach($data as $incomeReport): ?>
							    <tr>
							      <td><?= $i; ?></td>
							      <?php $cate=$this->db_model->getWhere('income_category',['id'=>$incomeReport['income_category']]); ?>
							      <td><?= $cate['name']; ?></td>
							      <td><?= $incomeReport['amount']; ?></td>
							      <td><?=$this->db_model->dateFormat($incomeReport['entry_date']);?></td>
							      <td><?= $incomeReport['remark']; ?></td>
							      <td>
							      	<button onclick="return printVoucher('<?=$this->db_model->dateFormat($incomeReport['entry_date']);?>','<?=$cate['name']; ?>','<?= $incomeReport['remark']; ?>','<?= $incomeReport['amount']; ?>','<?= $incomeReport['id']; ?>');" class="btn btn-primary btn-sm"> <i class="fa fa-print"></i></button> 
							      	<a class="btn btn-danger btn-sm" onclick="return confirm('Are you sure to delete.')" href="<?= base_url('accountant/deleteData/income_entry/'.$incomeReport['id'].'/income_management'); ?>"><span class="glyphicon glyphicon-trash"></span></a> 
							      </td>
							    </tr>
							   <?php $i++; endforeach; ?>
							   <?php } else { ?>
                ...
                <div class="modal-body" style="padding: 0px;">
                  	<div class="panel panel-primary" style="margin-bottom: 0px;">
		                ...
		                <div class="panel-body" id="myTable">
		                	<div class="table-responsive" style="border: 1px solid; padding: 15px;">
							...
							<tr align="center">
								<td colspan='5'><?=$this->siteInfo['address'];?><br>
									<b>income Voucher</b></td>
							</tr>
							</table>
                  			<table class="table table-bordered" width="100%">
                  				<tr>
                  					<td style="min-width: 150px;"><b>income Category : </b></td>
                  					<td class="incomeGroup"></td>
                  					<td style="min-width: 100px;"><b>Print Date : </b></td>
                  					<td style="min-width: 100px;"><?=date('d-M-Y');?></td>
                  				</tr>
                  				<tr>
                  					<td><b>Amount : </b></td>
                  					<td class="incomeAmount"></td>
                  					<td><b>Entry Date : </b></td>
                  					<td class="incomeDate"></td>
                  				</tr>
                  				<tr>
                  					<td><b>Description : </b></td>
                  					<td class="incomeDetail"></td>
							...
							<tr align="center">
								<td colspan='5'><?=$this->siteInfo['address'];?><br>
									<b>income Voucher (For Office Use Only)</b></td>
							</tr>
							</table>
                  			<table class="table table-bordered" width="100%">
                  				<tr>
...
<script type="text/javascript">
	...
	$('.incomeDate').html(date);
	$('.incomeGroup').html(group);
	$('.incomeDetail').html(detail);
	$('.incomeAmount').html(amount);
	$('.incomeId').html(id);
