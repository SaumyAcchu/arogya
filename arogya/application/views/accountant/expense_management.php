	      	...
	      	<div class="container-fluid">
		        ...
		        <div class="row padding-top">
					...
					<div class="col-lg-6 col-lg-offset-3" style="padding: 10px; box-shadow: 0px 1px 10px 1px #33A5E7; margin-top: 10px;">
						<?= form_open('accountant/expenseManagement/add',['class'=>'form-horizontal']); ?>
						...
						<div class="col-lg-10 col-lg-offset-1">
							...
							<table id="table1" class="table table-bordered table-striped table-hover table-condensed" style="margin-top: 30px;">
							  ...
							  <?php $i=1; foreach($data as $expenseReport): ?>
							    <tr>
							      <td><?= $i; ?></td>
							      <?php $cate=$this->db_model->getWhere('expense_category',['id'=>$expenseReport['expense_category']]); ?>
							      <td><?= $cate['name']; ?></td>
							      <td><?= $expenseReport['amount']; ?></td>
							      <td><?=$this->db_model->dateFormat($expenseReport['entry_date']);?></td>
							      <td><?= $expenseReport['remark']; ?></td>
							      <td>
							      	<button onclick="return printVoucher('<?=$this->db_model->dateFormat($expenseReport['entry_date']);?>','<?=$cate['name']; ?>','<?= $expenseReport['remark']; ?>','<?= $expenseReport['amount']; ?>','<?= $expenseReport['id']; ?>');" class="btn btn-primary btn-sm"> <i class="fa fa-print"></i></button> 
							      	<a class="btn btn-danger btn-sm" onclick="return confirm('Are you sure to delete.')" href="<?= base_url('accountant/deleteData/expense_entry/'.$expenseReport['id'].'/expense_management'); ?>"><span class="glyphicon glyphicon-trash"></span></a> 
							      </td>
							    </tr>
							   <?php $i++; endforeach; ?>
							   <?php } else { ?>
