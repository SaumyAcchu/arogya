...
<html lang="en">
<head>
  	<title><?=$page['page']='Purchase History';?> | <?=$this->siteInfo['name'];?></title>
		        ...
		        <div class="row padding-top">
		            <div class="col-lg-10 col-lg-offset-1">
						<table class="table table-striped table-hover table-bordered">
						   <tbody>
						   <?= form_open('auth/purchaseHistory'); ?>
						    <tr>
						      <td>
						        <div class="input-group" id="datepairExample">
						           <span class="input-group-addon" id="basic-addon1">User Id</span>
						            <input type="text" value="" name="user_id" id='user_id' class="form-control" required>
						        </div>
						      </td>
					  	...
					  	<table class="table table-bordered table-hover" id="table1">
					  		...
					  		<?php if($data){ $i=1;$total=0; foreach ($data as $key => $value)
					  		{
					  			...
					  			<tr>
					  				<td><?=$i;?></td>
					  				<td><?=$value['user_id'];?></td>
					  				<?php $date1=new DateTime($value['date']);
					  				$total=$total+$value['total_price']; ?>
					  				<td><?=$date1->format('d-M-Y');?></td>
					  				<td><?=$value['remark'];?></td>
					  				<td style="text-align: right;"><?=$value['total_price'];?></td>
					  				<td><a class="btn btn-warning" href="<?= base_url('auth/bookingSlip/'.$value['id']) ?>">More</a></td>
					  			</tr>	
					  		<?php $i++; } ?>
					  		<tfoot>
					  		<tr class="bg-info">
					  			<td></td><td></td><td></td><th>Total</th><td style="text-align: right;"><?=$total;?></td>
					  		</tr>
					  	</tfoot>
					  		<?php } else { ?>
					  		<tr>
