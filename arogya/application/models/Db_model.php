...
class Db_model extends CI_Model
{
	...
	public function return_print($condition='')
	{
		$query=$this->db->get_where('form',$condition)->row_array();
  //echo "<per>";print_r($query);die();
	return $query;
	...
	public function booked_print($id='') 
{	
$q=$this->db->select('*')->where($id)->get('form');
      return $q->row_array();
}
...
// 		$this->db->ORDER_BY('flat_num','ASC');
	...
	public function sendSms($mobile='',$msg='')
	{
		...
		// curl_setopt($ch, CURLOPT_POSTFIELDS, "user=".$username."&password=".$pass."&msisdn=".$mobile."&sid=".$sender."&msg=".$msg."&fl=0"."&gwid=2");
	...
	public function due_Installment_Notify($value='')
	{	
		$today = date('Y-m-d');
		$date_diff = 7;
		$date1=date_create($today);
		date_add($date1,date_interval_create_from_date_string('"'.$date_diff.' Days "'));
		$dateFrom= date_format($date1,"Y-m-d");
		$time_slot=['due_date>='=>$today,'due_date<='=>$dateFrom,'status'=>0,'is_cancelled'=>0];
		$installmentdata=$this->db_model->globalSelect('installment',$time_slot);
		return $installmentdata;
		
	}


	public function cancellation_Notify($value='')
	{	
		$today = date('Y-m-d');
		$date_diff = 15;
		$date1 = date_create($today);
		date_add($date1,date_interval_create_from_date_string('"'.$date_diff.' Days "'));
		$dateFrom = date_format($date1,"Y-m-d");
		$time_slot = ['next_due_date>='=>$today,'next_due_date<='=>$dateFrom,'status'=>1,'booking_status'=>1];
		$cancellationdata = $this->db_model->globalSelect('flat_registration',$time_slot);
		return $cancellationdata;		
	}

	public function installmentManageLow($data='',$amt='')
	{

		$condition = ['flat_user_id'=>$data['flat_user_id'],'status'=>0,'is_cancelled'=>0,'id!='=>$data['id']];
		$installmentdata = $this->db_model->globalSelect('installment',$condition);
		$dvamt = $data['emi'] + (($data['emi']-$amt)/count($installmentdata));
		foreach ($installmentdata as $key => $value) {
			$this->db_model->globalUpdate('installment',['id'=>$value['id']],array('emi'=>$dvamt));
		}
		
	}

	public function installmentManageHigh($data='',$amt='')
	{
		$condition = ['flat_user_id'=>$data['flat_user_id'],'status'=>0,'is_cancelled'=>0,'id!='=>$data['id']];
		$installmentdata = $this->db_model->globalSelect('installment',$condition);
		$dvamt = $data['emi'] - (($amt-$data['emi'])/count($installmentdata));
		foreach ($installmentdata as $key => $value) {
			$this->db_model->globalUpdate('installment',['id'=>$value['id']],array('emi'=>$dvamt));
		}
	}
