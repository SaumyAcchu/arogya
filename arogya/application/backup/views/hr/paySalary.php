...
<html lang="en">
<head>
  	<title><?=$page['page']='Pay Salary';?> | <?=$this->siteInfo['name'];?></title>
  	...
  	<div class="wrapper" id="wrapper">
	    ...
	    <div class="right-container" id="right-container">
	      	<div class="container-fluid">
                ...
                <?= form_open('hr/salaryPayed',['class'=>'form-horizontal']); ?>
        ...
        <div class="col-sm-12" id="myDiv">
            <?php  $total=0; if(isset($salaryReport))
            {
            ...
            <table id="example" class="table table-striped table-hover table-bordered display" border="1" cellpadding="5" style="width:100%;">
             ...
             <tbody>
             <?php $i=1; $j=0 ; $k=0; if(count($salaryReport)>0){ foreach($salaryReport as $list){ ?>
               ...
               <tr>
                   ...
                   <td><?=$list['is_active'];?></td>
                   <td><?=$list['date'];?></td>
                   <?php intval($list['salary']) ?>   
                   <?php $total = $total+intval($list['salary']); ?>
               </tr>
               <?php $k++; } $i++; } ?>
               <tr>
                 <th colspan="5"> <center> Total Prasent Dayes <?= $k ?> And total Absent days <?= $j ?></center></th><!-- <th>&#8377; <?php $amtunt=intval($total); echo $amtunt;?>/-</th> -->
               </tr>
               <!-- <tr>
                 <th>In words</th>
                 <td colspan="8">
           ...
           <?php if ($total>0): ?>
            ...
            <div class="col-sm-12">
             <table class="table table-hover table-bordered table-striped" border="1" cellpadding="5" style="width:100%;">
                             <?= form_open('hr/ganrateSalry'); ?>

                             <input type="hidden" name="emp_id" value="<?= $searchFor['employee_id'] ?>">
                             <?php 
                             $dateFrom=new DateTime($searchFor['dateFrom']);
                             $newdateFrom=$dateFrom->format('Y-m-d');
                             $showDate=$dateFrom->format('M-Y');
                                            
                              ?>
                             <input type="hidden" name="date_for" value="<?= $newdateFrom ?>" >
                             
                             <input type="hidden" name="totaldays" value="<?= $k + $j; ?>">
                             <input type="hidden" name="present" value="<?= $k; ?>">
                             <input type="hidden" name="absent" value="<?= $j; ?>">
                             <tbody>
                               <tr>
                                 <td colspan="6" style="color: red;">Generate Salary</td>
                               </tr>
                               <tr>
                                 <th>Employee Id</th><td colspan="2" style="text-transform: uppercase;"><?= $searchFor['employee_id'] ?></td>
                                 <th>Employee Name</th><td colspan="2" style="text-align: left;"><?=$employee['name'];?></td>
                               </tr>
                               <tr>
                                 <th>Salary For</th><td colspan="2"><?=$showDate;?></td>
                                 <th>Payed Date</th> <td colspan="2"> 
                                  <div class="input-group" id="datepairExample1">
                                    
                                     <input type="text" value="<?=date('d-m-Y');?>" name="generat_date" class="form-control date" id="datepicker" placeholder="dd-mm-yyyy" required>
                                  </div>

                                 </td>
                               </tr>
                                <th>Monthaly Salary</th> <td colspan="2"><input type="text" class="form-control" name="monthly_salary" value="<?=$employee['salary'];?>" readonly="readonly"></td>
                                 <th>Total Salery</th><td colspan="2" style="text-align: left;"><input type="number" class="form-control" name="salary" value="<?= $total ?>" required></td>
                               <tr>
                                 <th>Total Prasent</th><td colspan="2"><?= $k ?> Days</td>
                                 <th>Total Absent</th><td colspan="2"><?= $j ?> Days</td>
                               </tr>
                               <tr>
                                 <th colspan="4"></th><td> <button type="submit" onclick="" id="submit" class="btn btn-primary">Generate Salary</button></td>
                               </tr>
                             </tbody>
                             </form>
              </table>
