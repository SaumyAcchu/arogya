...
class Real1 extends MY_Controller
{
	...
	public function flatMgmt($param1='',$param2='',$param3='')
	{
		...
		if($param1=='add')
		{
			...
			for ($i=1; $i <= $data['same_flat']; $i++)
			{
				...
				$valid=$this->db_model->rowCount('flat',['flat_num'=>$check]);
				if($valid>0)
				{
	...
	public function flatReg($param1='',$param2='',$param3='')
	{
		...
		if($param1=='regsub')
		{
			...
			}else
			{
				...
				if($insertid)
				{
					...
					$data['pay_mode'] = 'Cash';
					$data['pay_date'] = $customer['registration_date'];
					...
					$msg="Dear ".$customer['name']." Booking Confirmed, Your Login ID ".$customer['registration']." and Password is : ".$customer['password']." http://srgroup.org.in/";
		...
		if($param1=='pay')
		{
			...
			$data['registration']=$data1['registration'];
			$data['flat_user_id']=$data1['flat_user_id'];
			$data['flat_id']=$data1['flat_id'];
			$data['building_id']=$data1['building_id'];
			$data['grand_total']=$data1['grand_total'];
			$data['sponcer']=$data1['sponcer'];
			$data['pay_date']=$data1['pay_date'];
			$data['emi']=$data1['emi'];
			$data['emi_month']=$data1['emi_month'];
			$data['plc']=$data1['plc'];
			$data['dev_charg']=$data1['dev_charg'];

			
			if($data1['pay_mode']=='Cash')
			{
				$amt=$data1['cas_pay_amount'];
			}
			elseif ($data1['pay_mode']=='Cheque') {
				$amt=0;
				foreach ($data1['pay_amount'] as $key => $value)
				{
					$amt=$amt+$value;
				}
				$data['cheque_num']=implode(',',$data1['cheque_num']);
				$data['cheque_detail']=implode(',',$data1['cheque_detail']);
				$data['pay_amount']=implode(',',$data1['pay_amount']);
				$data['trns_date']=implode(',',$data1['trns_date']);

			}
			elseif($data1['pay_mode']=='Rtgs')
			{
				
				$amt=$data1['pay_amount'];
				$data['cheque_num']=$data1['rtgs_num'];
				$data['cheque_detail']=$data1['rtgs_detail'];
				$data['pay_amount']=$data1['rtgs_pay_amount'];
				$data['trns_date']=$data1['rtgs_trns_date'];
			}
			elseif($data1['pay_mode']=='Nft')
			{
			...
			{
				$left=($data['grand_total']-$data['discount']+$data['plc']+$data['dev_charg'])-$amt;
			...
			$this->db_model->globalUpdate('flat_registration',['id'=>$data['flat_user_id']],array('emi'=>'yes','discount'=>$data['discount'],'plc'=>$data['plc'],'dev_charg'=>$data['dev_charg'],'status'=>1,'booking_status'=>1));
			$data['inst_amt']=$amt;
			$data['inst_num']='down';
			// echo "<pre>"; print_r($data); die;
	...
	{
			$data=$_POST;
			// echo "<pre>";print_r($customer); print_r($data);die();
			$amt=$data['pay_amount'];
			$date=$data['pay_date'];
			$dateForm=new DateTime($date);
			$data['pay_date']=$dateForm->format('Y-m-d');
